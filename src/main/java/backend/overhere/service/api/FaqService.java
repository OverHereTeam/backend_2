...
public class FaqService {
    ...
    public FaqDetailPageResponseDto getFaqs(int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<Faq> faqPage = faqRepository.findAllByOrderByCreatedAtDesc(pageable);

        List<FaqDetailPageResponseDto.PageFaqDetailResponseDto> collect = faqPage.getContent().stream()
                .map(faq -> FaqDetailPageResponseDto.PageFaqDetailResponseDto.builder().faqId(faq.getId()).title(faq.getTitle()).content(faq.getTitle()).createdAt(faq.getCreatedAt()).updatedAt(faq.getUpdatedAt()).build())
                .collect(Collectors.toList());
        return new FaqDetailPageResponseDto(faqPage.getTotalPages(),collect);
    }
